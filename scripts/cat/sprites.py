...
class Sprites():
    ...
    def load_all(self):
        ...
        self.make_group('lineartwng', (0, 0), 'lineswng')
        self.make_group('lineartwng', (1, 0), 'lineswngdead')
        ...
        for a, i in enumerate(
            ...
            self.make_group('extrawng', (a, 0), f'eyes_wng{i}')
        for a, i in enumerate(
                ['YELLOW', 'AMBER', 'HAZEL', 'PALEGREEN', 'GREEN', 'BLUE','DARKBLUE',
                 'GREY', 'CYAN', 'EMERALD', 'HEATHERBLUE', 'SUNLITICE', 'SUNSETICE', 'PURPLE']):
            self.make_group('extrawng', (a, 2), f'eyes2_wng{i}')
        for a, i in enumerate(
                ['COPPER', 'SAGE', 'COBALT', 'PALEBLUE', 'BRONZE', 'SILVER', 'PALEYELLOW',
                 'GOLD', 'GREENYELLOW', 'CHOCOLATE', 'RED', 'PINK', 'WARMGRAY', 'PANSY']):
            self.make_group('extrawng', (a, 1), f'eyes_wng{i}')
        for a, i in enumerate(
                ['COPPER', 'SAGE', 'COBALT', 'PALEBLUE', 'BRONZE', 'SILVER', 'PALEYELLOW',
                 'GOLD', 'GREENYELLOW', 'CHOCOLATE', 'RED', 'PINK', 'WARMGRAY', 'PANSY']):
            self.make_group('extrawng', (a, 3), f'eyes2_wng{i}')

        #white patches
        for a, i in enumerate(['FULLWHITE', 'ANY', 'TUXEDO', 'LITTLE', 'COLOURPOINT', 'VAN', 'ANYTWO',
            'MOON', 'PHANTOM', 'POWDER', 'BLEACHED', 'SAVANNAH', 'FADESPOTS', 'PEBBLESHINE']):
            self.make_group('extrawng', (14+a, 0), f'white_wng{i}')
        for a, i in enumerate(['EXTRA', 'ONEEAR', 'BROKEN', 'LIGHTTUXEDO', 'BUZZARDFANG', 'RAGDOLL', 
            'LIGHTSONG', 'VITILIGO', 'BLACKSTAR', 'PIEBALD', 'CURVED', 'PETAL', 'SHIBAINU', 'OWL']):
            self.make_group('extrawng', (14+a, 1), f'white_wng{i}')
        # ryos white patches
        for a, i in enumerate(['TIP', 'FANCY', 'FRECKLES', 'RINGTAIL', 'HALFFACE', 'PANTSTWO', 'GOATEE', 'VITILIGOTWO',
            'PAWS', 'MITAINE', 'BROKENBLAZE', 'SCOURGE', 'DIVA', 'BEARD']):
            self.make_group('extrawng', (14+a, 2), f'white_wng{i}')
        for a, i in enumerate(['TAIL', 'BLAZE', 'PRINCE', 'BIB', 'VEE', 'UNDERS', 'HONEY',
            'FAROFA', 'DAMIEN', 'MISTER', 'BELLY', 'TAILTIP', 'TOES', 'TOPCOVER']):
            self.make_group('extrawng', (14+a, 3), f'white_wng{i}')
        for a, i in enumerate(['APRON', 'CAPSADDLE', 'MASKMANTLE', 'SQUEAKS', 'STAR', 'TOESTAIL', 'RAVENPAW',
                'PANTS', 'REVERSEPANTS', 'SKUNK', 'KARPATI', 'HALFWHITE', 'APPALOOSA', 'DAPPLEPAW']):
            self.make_group('extrawng', (14+a, 4), f'white_wng{i}')
        for a, i in enumerate(['HEART', 'LILTWO', 'GLASS', 'MOORISH', 'SEPIAPOINT', 'MINKPOINT', 'SEALPOINT',
            'MAO', 'LUNA', 'CHESTSPECK', 'WINGS', 'PAINTED', 'HEARTTWO', 'WOODPECKER']):
        ...
        for a, i in enumerate(['BULLSEYE', 'FINN', 'DIGIT', 'KROPKA', 'FCTWO', 'FCONE', 'MIA', 'SCAR',
            'BUSTER', 'SMOKEY', 'HAWKBLAZE', 'CAKE', 'ROSINA', 'PRINCESS']):
            self.make_group('extrawng', (14+a, 8), f'white_wng{i}')
        for a, i in enumerate(['LOCKET']):
            self.make_group('extrawng', (14+a, 9), 'white_wng' + i)

        # single (solid)
        for a, i in enumerate(['WHITE', 'PALEGREY', 'SILVER', 'GREY', 'DARKGREY', 'GHOST', 'BLACK']):
            self.make_group('mainwng', (14+a, 0), f'single_wng{i}')
        for a, i in enumerate(['CREAM', 'PALEGINGER', 'GOLDEN', 'GINGER', 'DARKGINGER', 'SIENNA']):
            self.make_group('mainwng', (14+a, 1), f'single_wng{i}')
        for a, i in enumerate(['LIGHTBROWN', 'LILAC', 'BROWN', 'GOLDEN-BROWN', 'DARKBROWN', 'CHOCOLATE']):
            self.make_group('mainwng', (14+a, 2), f'single_wng{i}')
        # singlestripe
        for a, i in enumerate(['WHITE', 'PALEGREY', 'SILVER', 'GREY', 'DARKGREY', 'GHOST', 'BLACK']):
        ...
        for a, i in enumerate(['CREAM', 'PALEGINGER', 'GOLDEN', 'GINGER', 'DARKGINGER', 'SIENNA']):
            self.make_group('mainwng', (14+a, 7), f'smoke_wng{i}')
        for a, i in enumerate(['LIGHTBROWN', 'LILAC', 'BROWN', 'GOLDEN-BROWN', 'DARKBROWN', 'CHOCOLATE']):
            self.make_group('mainwng', (14+a, 8), f'smoke_wng{i}')
        # ticked
        for a, i in enumerate(['WHITE', 'PALEGREY', 'SILVER', 'GREY', 'DARKGREY', 'GHOST', 'BLACK']):
            self.make_group('mainwng', (a, 3), f'ticked_wng{i}')
        for a, i in enumerate(['CREAM', 'PALEGINGER', 'GOLDEN', 'GINGER', 'DARKGINGER', 'SIENNA']):
            self.make_group('mainwng', (a, 4), f'ticked_wng{i}')
        for a, i in enumerate(['LIGHTBROWN', 'LILAC', 'BROWN', 'GOLDEN-BROWN', 'DARKBROWN', 'CHOCOLATE']):
            self.make_group('mainwng', (a, 5), f'ticked_wng{i}')
        # agouti
        for a, i in enumerate(['WHITE', 'PALEGREY', 'SILVER', 'GREY', 'DARKGREY', 'GHOST', 'BLACK']):
            self.make_group('mainwng', (7+a, 3), f'agouti_wng{i}')
        for a, i in enumerate(['CREAM', 'PALEGINGER', 'GOLDEN', 'GINGER', 'DARKGINGER', 'SIENNA']):
            self.make_group('mainwng', (6+a, 4), f'agouti_wng{i}')
        for a, i in enumerate(['LIGHTBROWN', 'LILAC', 'BROWN', 'GOLDEN-BROWN', 'DARKBROWN', 'CHOCOLATE']):
            self.make_group('mainwng', (6+a, 5), f'agouti_wng{i}')


        #pigeon
        for a, i in enumerate(['SILVER', 'OPAL', 'GREY', 'DARKGREY', 'LIGHTBLUE', 'BLUE', 'DARKBLUE']):
           self.make_group('mainwng', (a, 0), f"pigeonbar{i}")
        for a, i in enumerate(['CREAM', 'FAWN', 'CINNAMON', 'LIGHTRED', 'RED', 'BROWN', 'DARKBROWN']):
            self.make_group('mainwng', (a, 1), f"pigeonbar{i}")

        for a, i in enumerate(['SILVER', 'OPAL', 'GREY', 'DARKGREY', 'LIGHTBLUE', 'BLUE', 'DARKBLUE']):
            self.make_group('mainwng', (7+a, 0), f"pigeoncheck{i}")
        for a, i in enumerate(['CREAM', 'FAWN', 'CINNAMON', 'LIGHTRED', 'RED', 'BROWN', 'DARKBROWN']):
            self.make_group('mainwng', (7+a, 1), f"pigeoncheck{i}")

        for a, i in enumerate(['SILVER', 'FAWN', 'CINNAMON', 'BROWN', 'BLUE', 'DARKBLUE', 'BLACK']):
            self.make_group('mainwng', (a, 2), f"pigeonspread{i}")

        for a, i in enumerate(['ICE', 'LARK', 'REDPEN', 'BLUEPEN', 'MOTTLEDLIGHT', 'MOTTLED', 'MOTTLEDDARK']):
            self.make_group('mainwng', (7+a, 2), f"pigeonfancy{i}")

        #tortie patches
        for a, i in enumerate(['ONE', 'TWO', 'THREE', 'FOUR', 'REDTAIL', 'DELILAH', 'HALF', 'STREAK', 'MASK', 'MOTTLED', 'SIDEMASK', 'EYEDOT', 'BANDANA', 'PACMAN']):
            self.make_group('extrawng', (a, 9), f"tortiemask_wng{i}")
        for a, i in enumerate(['MINIMALONE', 'MINIMALTWO', 'MINIMALTHREE', 'MINIMALFOUR', 'OREO', 'SWOOP', 'CHIMERA', 'CHEST', 'ARMTAIL', 'STREAMSTRIKE', 'SMUDGED', 'DAUB', 'EMBER', 'ORIOLE']):
            self.make_group('extrawng', (a, 10), f"tortiemask_wng{i}")
        for a, i in enumerate(['ROBIN', 'BRINDLE', 'PAIGE', 'ROSETAIL', 'SAFI', 'DAPPLENIGHT', 'BLANKET', 'SMOKE', 'GRUMPYFACE', 'BRIE', 'BELOVED', 'BODY', 'SHILOH']):
        ...
        for a, i in enumerate(['BLACK', 'RED', 'PINK', 'DARKBROWN', 'BROWN', 'LIGHTBROWN', 'DARKMARBLED',
            'MARBLED', 'LIGHTMARBLED', 'DARKBLUE', 'BLUE', 'LIGHTBLUE', 'DARK', 'DARKGREY']):
            self.make_group('extrawng', (a, 4), f"skin_wng{i}")
        for a, i in enumerate(['DARKSALMON', 'SALMON', 'PEACH', 'GREY']):
            self.make_group('extrawng', (a, 5), f"skin_wng{i}")

        # WURM
        for a, i in enumerate(
                ['YELLOW', 'AMBER', 'HAZEL', 'PALEGREEN', 'GREEN', 'BLUE','DARKBLUE',
                 'GREY', 'CYAN', 'EMERALD', 'HEATHERBLUE', 'SUNLITICE', 'SUNSETICE', 'PURPLE']):
            self.make_group('extrawurm', (a, 0), f'eyes_wurm{i}')
        for a, i in enumerate(
                ['YELLOW', 'AMBER', 'HAZEL', 'PALEGREEN', 'GREEN', 'BLUE','DARKBLUE',
                 'GREY', 'CYAN', 'EMERALD', 'HEATHERBLUE', 'SUNLITICE', 'SUNSETICE', 'PURPLE']):
            self.make_group('extrawurm', (a, 2), f'eyes2_wurm{i}')
        for a, i in enumerate(
                ['COPPER', 'SAGE', 'COBALT', 'PALEBLUE', 'BRONZE', 'SILVER', 'PALEYELLOW',
                 'GOLD', 'GREENYELLOW', 'CHOCOLATE', 'RED', 'PINK', 'WARMGRAY', 'PANSY']):
            self.make_group('extrawurm', (a, 1), f'eyes_wurm{i}')
        for a, i in enumerate(
                ['COPPER', 'SAGE', 'COBALT', 'PALEBLUE', 'BRONZE', 'SILVER', 'PALEYELLOW',
                 'GOLD', 'GREENYELLOW', 'CHOCOLATE', 'RED', 'PINK', 'WARMGRAY', 'PANSY']):
            self.make_group('extrawurm', (a, 3), f'eyes2_wurm{i}')

        #white patches
        for a, i in enumerate(['FULLWHITE', 'ANY', 'TUXEDO', 'LITTLE', 'COLOURPOINT', 'VAN', 'ANYTWO',
            'MOON', 'PHANTOM', 'POWDER', 'BLEACHED', 'SAVANNAH', 'FADESPOTS', 'PEBBLESHINE']):
            self.make_group('extrawurm', (14+a, 0), f'white_wurm{i}')
        for a, i in enumerate(['EXTRA', 'ONEEAR', 'BROKEN', 'LIGHTTUXEDO', 'BUZZARDFANG', 'RAGDOLL', 
            'LIGHTSONG', 'VITILIGO', 'BLACKSTAR', 'PIEBALD', 'CURVED', 'PETAL', 'SHIBAINU', 'OWL']):
            self.make_group('extrawurm', (14+a, 1), f'white_wurm{i}')
        for a, i in enumerate(['TIP', 'FANCY', 'FRECKLES', 'RINGTAIL', 'HALFFACE', 'PANTSTWO', 'GOATEE', 'VITILIGOTWO',
            'PAWS', 'MITAINE', 'BROKENBLAZE', 'SCOURGE', 'DIVA', 'BEARD']):
            self.make_group('extrawurm', (14+a, 2), f'white_wurm{i}')
        for a, i in enumerate(['TAIL', 'BLAZE', 'PRINCE', 'BIB', 'VEE', 'UNDERS', 'HONEY',
            'FAROFA', 'DAMIEN', 'MISTER', 'BELLY', 'TAILTIP', 'TOES', 'TOPCOVER']):
            self.make_group('extrawurm', (14+a, 3), f'white_wurm{i}')
        for a, i in enumerate(['APRON', 'CAPSADDLE', 'MASKMANTLE', 'SQUEAKS', 'STAR', 'TOESTAIL', 'RAVENPAW',
                'PANTS', 'REVERSEPANTS', 'SKUNK', 'KARPATI', 'HALFWHITE', 'APPALOOSA', 'DAPPLEPAW']):
            self.make_group('extrawurm', (14+a, 4), f'white_wurm{i}')
        for a, i in enumerate(['HEART', 'LILTWO', 'GLASS', 'MOORISH', 'SEPIAPOINT', 'MINKPOINT', 'SEALPOINT',
            'MAO', 'LUNA', 'CHESTSPECK', 'WINGS', 'PAINTED', 'HEARTTWO', 'WOODPECKER']):
            self.make_group('extrawurm', (14+a, 5), f'white_wurm{i}')
        for a, i in enumerate(['BOOTS', 'MISS', 'COW', 'COWTWO', 'BUB', 'BOWTIE', 'MUSTACHE', 'REVERSEHEART',
            'SPARROW', 'VEST', 'LOVEBUG', 'TRIXIE', 'SAMMY', 'SPARKLE']):
            self.make_group('extrawurm', (14+a, 6), f'white_wurm{i}')
        # acorn's white patches: the sequel
        for a, i in enumerate(['RIGHTEAR', 'LEFTEAR', 'ESTRELLA', 'SHOOTINGSTAR', 'EYESPOT', 'REVERSEEYE',
            'FADEBELLY', 'FRONT', 'BLOSSOMSTEP', 'PEBBLE', 'TAILTWO', 'BUDDY', 'BACKSPOT', 'EYEBAGS']):
            self.make_group('extrawurm', (14+a, 7), f'white_wurm{i}')
        for a, i in enumerate(['BULLSEYE', 'FINN', 'DIGIT', 'KROPKA', 'FCTWO', 'FCONE', 'MIA', 'SCAR',
            'BUSTER', 'SMOKEY', 'HAWKBLAZE', 'CAKE', 'ROSINA', 'PRINCESS']):
            self.make_group('extrawurm', (14+a, 8), f'white_wurm{i}')
        for a, i in enumerate(['LOCKET']):
            self.make_group('extrawurm', (14+a, 9), 'white_wurm' + i)
        ...
        for a, i in enumerate(['BLACK', 'RED', 'PINK', 'DARKBROWN', 'BROWN', 'LIGHTBROWN', 'DARKMARBLED', 'MARBLED', 'LIGHTMARBLED', 'DARKBLUE', 'BLUE', 'LIGHTBLUE', 'DARK', 'DARKGREY']):
            self.make_group('extrawurm', (a, 4), f"skin_wurm{i}")
        for a, i in enumerate(['DARKSALMON', 'SALMON', 'PEACH', 'GREY']):
            self.make_group('extrawurm', (a, 5), f"skin_wurm{i}")
